...
import { ChatbotContentStatusType, chatbotContentType, SendChatBotSchemaType, useCaseEnum } from '@/schemas/chatbot.index';
  ...
  return (
    <div className="grid h-full grid-rows-[50px_1fr] overflow-hidden">
      <div className="border-b px-5 py-2">
        <h3 className="font-bricolage text-[13px] font-medium text-foreground">{chatbot?.title ? chatbot?.title : 'Untitled Chatbot'}</h3>
        <p className="font-inter text-[10px] font-medium text-foreground/50">Chatbot Id: {chatbot?.id}</p>
      </div>

      <div className="grid h-full grid-cols-2 grid-rows-1 overflow-hidden">
        <Tabs defaultValue="create" className="grid h-full grid-rows-[40px_1fr] border-r border-border p-0 shadow-[4px_0px_20px_0px_#0000000A]">
          ...
          <TabsContent value="create">
            <div className="mt-0 grid h-full">
              <Form {...form}>
                <form className="grid h-full grid-rows-[1fr_56px]" onSubmit={onSubmit}>
                  <div className="flex h-full flex-col gap-3 px-[23px] py-[31px]">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-bricolage text-[13px] font-medium text-foreground">Title</FormLabel>
                          <FormControl>
                            <Input
                              {...field}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-bricolage text-[13px] font-medium text-foreground">Use Case</FormLabel>
                          <Select onValueChange={(e) => onChange(e, field.name)} value={chatbot.useCase || field.value}>
                            <FormControl>
                              <SelectTrigger className="rounded-none border border-black/35 font-inter text-[13px] font-medium text-foreground/65">
                            ...
                            <SelectContent>
                              <SelectItem value={useCaseEnum.Automate}>Automate</SelectItem>
                              <SelectItem value={useCaseEnum.Chat}>Chat</SelectItem>
                              <SelectItem value={useCaseEnum.Manage}>Manage</SelectItem>
                              <SelectItem value={useCaseEnum.Schedule}>Schedule</SelectItem>
                              <SelectItem value={useCaseEnum.Support}>Support</SelectItem>
                            </SelectContent>
                          </Select>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-bricolage text-[13px] font-medium text-foreground">Global Prompt</FormLabel>
                          <FormControl>
                            <Textarea
                              {...field}
                  ...
                  <div className="flex w-full justify-end border-t border-border px-[14px] py-1.5">
                    <Button type="submit" variant="default" className="mt-auto px-[17px]">
                      Save
                    </Button>
                  </div>
                </form>
          ...
          <TabsContent value="knowledgeBase">
            <div className="mt-0 grid h-full grid-rows-[1fr_56px] overflow-hidden">
                ...
                <div className="overflow-y-auto scrollbar-thin scrollbar-track-secondary/10 scrollbar-thumb-secondary/65">
                  <div className="relative grid w-full grid-cols-[43%_auto_auto] text-left">
                    <div className="sticky top-0 col-span-3 grid grid-cols-subgrid">
                      <div className="border border-x-0 bg-white p-3 font-bricolage text-[13px] font-medium text-foreground">Document</div>
                      <div className="border bg-white p-3 font-bricolage text-[13px] font-medium text-foreground">Status</div>
                      <div className="border border-x-0 bg-white p-3 font-bricolage text-[13px] font-medium text-foreground">Details</div>
                    </div>

                    {chatbot?.chatbotContent.map((item: chatbotContentType, idx) => (
                      <div key={idx} className="col-span-3 grid grid-cols-subgrid">
                        <div className="cursor-pointer border-b border-border p-3 font-inter text-[13px] font-medium text-foreground/65">{item.id}</div>
                        <div className={cn('cursor-pointer border-b border-border p-3 font-inter text-[13px] font-medium', item.status === 'pending' ? 'text-red-500' : 'text-green-500')}>
                          {item.status === 'pending' ? 'PENDING' : 'Completed'}
                        </div>
                        <div className="cursor-pointer border-b border-border p-3 font-inter text-[13px] font-medium">
                          <Button type="button" onClick={() => HandleDocsSheet(idx)} variant="secondaryOutline" size="small" className="flex items-center gap-1">
                            View
              ...
              <div className="flex w-full justify-end border-t border-border px-[14px] py-1.5">
                <Button type="submit" variant="primary" className="ml-auto px-[17px]" onClick={onUploadFilesToVectorDatabase}>
                  Upload
                </Button>
              </div>
                ...
                <SheetContent className="grid h-screen grid-rows-[56px_1fr_40px] gap-0 p-0 sm:max-w-[547px]">
                  <SheetHeader className="border-b border-border">
                    <SheetTitle className="px-3 py-2 font-bricolage text-[13px] font-medium text-foreground/65">
                      {docIndex !== null ? chatbot.chatbotContent[docIndex].id : null}
                      <p className="font-inter text-[10px] font-medium text-foreground/50">Document Id: {docIndex !== null ? chatbot.chatbotContent[docIndex].id : null}</p>
                    </SheetTitle>
                  </SheetHeader>
                  <div className="border-border px-[13px] py-2.5">
                    ...
                    <div className="flex items-center justify-between border-b border-border px-[5px] py-2.5 font-inter text-[13px] font-medium text-foreground">
                      <p>Status</p>
                      <p className={cn('uppercase text-foreground/65', docIndex !== null && chatbot.chatbotContent[docIndex]?.status === 'pending' ? 'text-red-500' : 'text-green-500')}>
                        {docIndex !== null ? chatbot.chatbotContent[docIndex].status : null}
                      </p>
                    </div>
                    <div className="flex items-center justify-between px-[5px] py-2.5 font-inter text-[13px] font-medium text-foreground">
                      <p>Created At</p>
                      <p className="text-foreground/65">{docIndex !== null ? format(new Date(chatbot.chatbotContent[docIndex].createdAt ?? ''), 'PP') : null}</p>
                    </div>
                    ...
                    <div className="h-[calc(100vh-265px)] w-full overflow-hidden border border-border sm:w-[520px]">
                      <div className="w-full border-b px-[15px] py-[18px]">
                        <p className="font-bricolage text-[13px] font-medium text-foreground">Content</p>
                      </div>
                      <div className="h-[calc(100%-56px)] w-full overflow-y-auto border-border px-3 py-5 scrollbar-thin scrollbar-track-secondary/10 scrollbar-thumb-secondary/65">
                        {docIndex !== null ? chatbot.chatbotContent[docIndex].content : null}
                      </div>
                    </div>
        ...
        {chatbot?.id ? <ChatBox chatbotId={chatbot?.id || ''} /> : <div className="h-full w-full bg-[#FD5B1D05]"></div>}
      </div>
    </div>
  );
}
