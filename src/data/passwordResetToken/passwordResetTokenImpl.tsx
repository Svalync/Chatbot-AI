...
export default class passwordResetTokenImpl
  implements passwordResetTokenInterface
{
  id: string = '';
  email: string = '';
  token: string = '';
  ...
  async generateVerificationToken(email: string) {
    ...
    const passwordResetTokenControllerHandler =
      new passwordResetTokenController();
    const existingToken: any =
      await passwordResetTokenControllerHandler.getPasswordResetTokenByEmail(
        email,
      );

    if (existingToken) {
      this.initFromDataObject(existingToken);
      await passwordResetTokenControllerHandler.deletePasswordResetTokenById(
        this.id,
      );
    }

    const verficationToken = await passwordResetTokenControllerHandler.create(
      email,
      token,
      expires,
    );
