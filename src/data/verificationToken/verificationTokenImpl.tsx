...
export default class verificationTokenImpl
  implements verificationTokenInterface
{
  id: string = '';
  email: string = '';
  token: string = '';
  ...
  async generateVerificationToken(email: string) {
    ...
    const vertificationTokenControllerHandler =
      new verficationTokenController();
    const existingToken: any =
      await vertificationTokenControllerHandler.getVerificationTokenByEmail(
        email,
      );

    if (existingToken) {
      this.initFromDataObject(existingToken);
      await vertificationTokenControllerHandler.deleteVerificationTokenById(
        this.id,
      );
    }

    const verficationToken = await vertificationTokenControllerHandler.create(
      email,
      token,
      expires,
    );
